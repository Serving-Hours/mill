import { getLinksByUserId } from "@/db/queries/select";
import { nunito } from "@/app/ui/fonts";
import { IconCopy, IconLink } from "@tabler/icons-react";
import Link from "next/link";
import DeleteLinkButton from "./link-delete";
...
export default async function LinkCards({ userId }: { userId: string }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 mb-8">
      {links?.map((link) => (
        <div
          className="flex flex-col p-4 rounded-3xl bg-[#F7F7F7]"
          key={link.id}
        >
          ...
          <Link
            target="_blank"
            href={link.url}
            className={`${nunito.className} text-lg`}
          >{`localhost:3000/${link.slug}`}</Link>
          <p className="font-medium text-sm truncate text-[#777777] mb-6">
            {link.url}
          </p>
          <div className="flex gap-2">
            <Link
              href={`/link/${link.id}/edit`}
              className="flex items-center p-3 h-[32px] w-fit bg-[#EAEAEA] text-[#777777] font-semibold rounded-full"
            >
              Edit
            </Link>
